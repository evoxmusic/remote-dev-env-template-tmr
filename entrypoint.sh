...
#   BLUEPRINT_CHAT_SYSTEM_PROMPT — Custom AI assistant instructions from blueprint settings
#   BLUEPRINT_DATA_SOURCES_PROMPT — Auto-generated data source descriptions for AI context
...
# ── Generate AGENTS.md from dynamic environment configuration ────────────────
# Assembled on every container start from env vars injected by the RDE portal:
#   BLUEPRINT_CHAT_SYSTEM_PROMPT  — admin-configured system prompt
#   BLUEPRINT_DATA_SOURCES_PROMPT — auto-generated data source descriptions
#   RDE_USER_PROFILE              — JSON user profile for AI personalization
generate_agents_md() {
  local agents_file="$PROJECT_DIR/AGENTS.md"

  # Skip if none of the dynamic env vars are set
  if [[ -z "${BLUEPRINT_CHAT_SYSTEM_PROMPT:-}" ]] && \
     [[ -z "${BLUEPRINT_DATA_SOURCES_PROMPT:-}" ]] && \
     [[ -z "${RDE_USER_PROFILE:-}" ]]; then
    return
  fi

  echo "Generating AGENTS.md from environment configuration..."
...
HEADER
  ...
  if [[ -n "${BLUEPRINT_CHAT_SYSTEM_PROMPT:-}" ]]; then
    echo "$BLUEPRINT_CHAT_SYSTEM_PROMPT" >> "$agents_file"
    echo "" >> "$agents_file"
  fi

  # Data source descriptions (auto-generated by the portal)
  if [[ -n "${BLUEPRINT_DATA_SOURCES_PROMPT:-}" ]]; then
    echo "$BLUEPRINT_DATA_SOURCES_PROMPT" >> "$agents_file"
    echo "" >> "$agents_file"
  fi

  # User profile context for AI personalization
  if [[ -n "${RDE_USER_PROFILE:-}" ]]; then
    local comfort lang role tz
    comfort=$(echo "$RDE_USER_PROFILE" | jq -r '.code_comfort // "familiar"')
    lang=$(echo "$RDE_USER_PROFILE" | jq -r '.preferred_language // ""')
    role=$(echo "$RDE_USER_PROFILE" | jq -r '.role_title // ""')
    tz=$(echo "$RDE_USER_PROFILE" | jq -r '.timezone // ""')

    echo "## User Profile" >> "$agents_file"
    echo "" >> "$agents_file"
    echo "- Comfort with code: $comfort" >> "$agents_file"
    [[ -n "$lang" ]] && echo "- Preferred language: $lang" >> "$agents_file"
    [[ -n "$role" ]] && echo "- Role: $role" >> "$agents_file"
    [[ -n "$tz" ]] && echo "- Timezone: $tz" >> "$agents_file"
    echo "" >> "$agents_file"

    # Behavioral instruction based on comfort level
    case "$comfort" in
      non_technical)
        cat >> "$agents_file" <<'EOF'
**Important:** This user is non-technical. Never show raw code unless explicitly asked. Explain everything in plain language. Focus on what the result does, not how it works. Provide step-by-step guidance with clear, simple instructions.
EOF
        ;;
      familiar)
        cat >> "$agents_file" <<'EOF'
**Note:** This user is familiar with code but may not be an expert. Explain technical decisions briefly. Show code when relevant but include context about what it does.
EOF
        ;;
      developer)
        cat >> "$agents_file" <<'EOF'
**Note:** This user is a developer. Be direct and technical. Show code freely. Skip basic explanations unless asked.
EOF
        ;;
    esac
  fi

  echo "Generated AGENTS.md with dynamic workspace instructions"
}
